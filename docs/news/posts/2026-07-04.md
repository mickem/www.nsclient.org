...
network checks. Around the daemon it delivers , a **secure-by-default web server**, one-command installs via winget /
Chocolatey / Scoop and `nscp web install-ui`, and a broad set of security and reliability fixes. It also hardens plugin
shutdown so a misbehaving module can no longer crash the service on exit.
...
choco install nsclient # Still pending review
