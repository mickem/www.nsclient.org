...
3. **Hardened by default** — `0.12.2` is a security release that closes listener defaults that used to be silently
   permissive (empty `allowed hosts`, plaintext `check_nt`, query-string tokens, etc.).
4. **Many long-standing check fixes** — `check_service`, `check_process`, `check_files`, `check_drivesize`,
   `check_uptime`, `CheckLogFile`, and the shared filter/threshold engine all behave correctly now.

> **Read the [Breaking changes](#breaking-changes) section before upgrading** — several long-standing-but-incorrect
...
- **New scenario: Checkmk agent integration.** Point a Checkmk site at port 6556 and you get a native-looking agent
  dump. See [scenarios/check-mk.md](docs/docs/scenarios/check-mk.md).
...
- **New REST endpoints**: `GET/DELETE /api/v2/events`, `GET /api/v2/metadata`, `DELETE /api/v2/settings/...`. Covered
  in [api/rest/](docs/docs/api/rest/).
...
* **`perf-syntax=none` actually suppresses perfdata** — `check_service` used to emit empty perfdata aliases (
...
* **#730** — `max-depth=0` now scans the top directory only (was: bail out before scanning anything, returning "no files
  found").
...
8. **`warn=` / `crit=` no longer fire mid-iteration on running counts.**
   ...
   ```
9. **Mixed `warn=` / `crit=` now evaluate when no rows match.**
   ```text
...
10. **`check_service`: `delayed` is no longer reported for non-auto services.** Filters that matched
    ...
    ```
11. **Realtime `check_process` is now case-insensitive.** A rule that intentionally matched only an exact casing will
    now match all variants (almost certainly the desired behaviour).
12. **`check_service`: `${desc}` no longer returns the literal `TODO`.** Use the real display name.
13. **`check_service`: `perf-syntax=none` actually suppresses perfdata.** Backends that consumed the empty-aliased
    entries (highly unlikely) will see them disappear.

### `check_files` — corner cases changed

14. **`max-depth=0`** now scans the top directory instead of returning empty (#730).
15. **Missing paths** now return UNKNOWN instead of OK / empty (#613).
16. **NTFS junction loops** are no longer double-counted (#605).
17. **Legacy `CheckFiles`** calls that previously returned UNKNOWN on empty results will now return OK (#717).

### Configuration / startup

18. **CheckExternalScripts: malformed alias commands are refused at startup.** The fallback "split-on-space" parser has
    been removed.
...
19. **HTTP request/response API changed.** Internal C++ types `http::request` / `http::response` are now distinct,
...
20. The documentation tree was restructured (`concepts/`, `checks-in-depth/`, `scenarios/`, `tutorial/`, `reference/`
...
[You can download the new version from GitHub](https://github.com/mickem/nscp/releases/0.13.3){ .md-button }
