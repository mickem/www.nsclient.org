...
# Layout. ROOT_DIR is the directory holding mkdocs.yml (the parent of this
# hooks/ directory) in both places the hook is used:
#
# * in the nscp repository that is <nscp>/docs: the notes live in
#   <nscp>/docs/upgrades and <nscp>/docs/security, the pages in the docs_dir
#   <nscp>/docs/docs at the site root (setup/upgrading.md), and the module
#   names come from <nscp>/modules;
# * on www.nsclient.org that is the site checkout itself: the notes live in
#   <site>/upgrades and <site>/security, the docs_dir is <site>/docs and the
#   agent documentation is copied into <site>/docs/docs, so the same pages sit
#   one level deeper (docs/setup/upgrading.md). PAGE_PREFIX is that level, and
#   there is no source tree, so module names come from the reference pages.
ROOT_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
UPGRADES_DIR = os.path.join(ROOT_DIR, 'upgrades')
SECURITY_DIR = os.path.join(ROOT_DIR, 'security')
PAGES_DIR = os.path.join(ROOT_DIR, 'docs')  # the mkdocs docs_dir
PAGE_PREFIX = 'docs/'  # '' in the nscp repository
MODULES_DIR = None  # os.path.join(os.path.dirname(ROOT_DIR), 'modules') in the nscp repository
REFERENCE_DIR = os.path.join(PAGES_DIR, PAGE_PREFIX, 'reference')

UPGRADING_PAGE = PAGE_PREFIX + 'setup/upgrading.md'
NOTICES_PAGE = PAGE_PREFIX + 'security/notices.md'
